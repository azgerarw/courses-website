...
router.post('/', validarSolicitud, async (req, res) => {
    ...
    if (!usuario)
        return res.status(404).json({ status: 404, body: { error: 'Usuario no encontrado' } });
    ...
    if (usuario[0]) {
        await db.run('UPDATE usuarios SET token_restablecer = ?, token_expira = ? WHERE usuario_id = ?', [token, expiracion, usuario[0].usuario_id]);
...
router.get('/:token', async (req, res) => {
    const db = await dbPromise;
    const usuario = await db.all('SELECT * FROM usuarios WHERE token_restablecer = ? AND token_expira > ?', [req.params.token, Date.now()]);
    if (!usuario) {
...
router.put('/', validarRestablecimiento, async (req, res) => {
    const errores = validationResult(req);
    if (!errores.isEmpty()) {
        const erroresFormateados = {};
        errores.array().forEach(error => {
    ...
    const db = await dbPromise;
    const usuario = await db.all('SELECT * FROM usuarios WHERE token_restablecer = ? AND token_expira > ?', [req.body.token, Date.now()]);
    if (!usuario)
        return res.status(400).json({ status: 400, body: { error: 'Token inválido o expirado' } });
    if (req.body.nuevaContrasena) {
        const hash = await bcrypt.hash(req.body.nuevaContrasena, 10);
        await db.run('UPDATE usuarios SET password = ?, token_restablecer = NULL, token_expira = NULL WHERE usuario_id = ?', [hash, usuario[0].usuario_id]);
