...
interface CustomRequest extends Request<{}, {}, CustomBody> {
...
export default async function registerUser(req: CustomRequest, res: Response<APIResponse>) {
    ...
    try {
        ...
        const usuario_existente = await db.get(
            `SELECT * FROM usuarios WHERE email = ?`,
            [email]
          );

          const usuario_existente2 = await db.get(
      ...
      await db.run(
        ...
        [usuario, email, hash, imagen, verificado, tokenVerificacion]
      );
